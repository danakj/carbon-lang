...
instructions, such as `SemIR::FacetType` that represents constraints on a type.
There's a singleton `FacetType` we call `TypeType` to represent the
unconstrained facet type `type`. Builtins have stable ids in the
`SemIR::InstStore` across `SemIR` instances.
