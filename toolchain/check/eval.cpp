...
static auto AddRequirementImpls(Context& context, SemIR::LocId loc_id,
  ...
  if (IsPeriodSelf(context, lhs_id)) {
    ...
    // FIXME: Test this, it was wrong. It didn't remap
    // `C impls (type where .Self impls X)` to say `C impls X`.
    llvm::append_range(
        info->type_impls_interfaces,
        llvm::map_range(rhs.self_impls_constraints, extends_interface));
    llvm::append_range(
        info->type_impls_named_constraints,
